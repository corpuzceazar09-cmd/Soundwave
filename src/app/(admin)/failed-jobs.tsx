...
export default function FailedJobsPage() {
  ...
  const fetchFailedFeeds = async () => {
    try {
      ...
      console.error('Failed feeds fetch error:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const retryFeed = async (feed: FailedFeed) => {
    ...
    try {
      ...
      setTimeout(async () => {
        const mockItems = Math.floor(Math.random() * 30) + 3;
        await supabase.from('ingestion_jobs').update({
          status: 'success',
          items_processed: mockItems,
          duration_ms: Math.floor(Math.random() * 10000) + 500,
  ...
  // Fetch also failed ingestion jobs
